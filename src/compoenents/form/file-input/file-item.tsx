...
const fileItem = tv({
  ...
  variants: {
    state: {
      progess: {
        cotainer: 'dark:border-zinc-700',
      },
      complete: {
        cotainer: 'border-violet-500 dark:border-violet-300/30',
      },
      error: {
        cotainer:
          'bg-red-25 border-error-300 dark:bg-error-500/5 dark:border-error-500/30',
        icon: 'border-error-50 bg-error-100 text-error-600 dark:bg-error-500/5 dark:border-error-500/30 dark:text-error-400',
        deleteButton:
          'text-red-700 hover:text-red-900 dark:text-error-400 dark:hover:text-red-300',
      },
    },
  },
  defaultVariants: {
...
export function FileItem({ name, size, state }: FileItemProps) {
  const { cotainer, icon, deleteButton } = fileItem({ state })
        ...
        <div className="flex flex-1 flex-col items-start gap-1">
          <div className="flex flex-col">
            <span className="text-sm font-medium text-red-700 dark:text-red-400">
              Upload falied, please try again.
            </span>
          ...
          <button
            type="button"
            className="text-error-700 dark:text-error-400 hover:text-error-900 dark:hover:text-error-300 text-sm font-semibold"
          >
            Try again
          </button>
        </div>
